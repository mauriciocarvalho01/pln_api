...
class ChatResponse:
    def getChatResponse(database, hash):
        database.execute("SELECT response FROM heroku_8a1f058f1608fe5.chat_response WHERE hash=%s", (hash,))
    ...
    def insertChatResponse(database, response, process):
        ...
        if len(check_exists) == 0: 
            insert = database.execute('INSERT INTO heroku_8a1f058f1608fe5.chat_response (hash, text, response) VALUES (%s,%s, %s)', (hash, text, response))
            print("Insert")
            print(insert)
            if(insert):
